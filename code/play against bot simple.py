...
class ChessApp:
    ...
    def create_game_interface(self):
        ...
        # Play Button, centered at the top of the mode frame
        self.play_button = tk.Button(self.mode_frame, text="▶ Spielen", font=("Arial", 20), bg="green", fg="white", command=self.start_game, width=10)
        self.play_button.pack(pady=(10, 5))  # Padding adjusted to place play button at the top
    
        # Description label centered below the play button
        self.description_label = tk.Label(self.mode_frame, text="Chessbot Br", font=("Comic Sans MS", 24, "bold"), bg="black", fg="white")
        self.description_label.pack(side="top", anchor="center", expand=True, padx=20, pady=(10, 5))  # Padding adjusted
        ...
        # Pawn color selection
        tk.Label(self.mode_frame, text="Farbe des Bauern wählen:", font=("Arial", 16), bg="black", fg="white").pack(anchor="w", padx=20, pady=5)
        tk.Radiobutton(self.mode_frame, text="Weiß", variable=self.pawn_color, value="white", font=("Arial", 16), bg="black", fg="gray", indicatoron=True).pack(anchor="w", padx=20, pady=5)
        tk.Radiobutton(self.mode_frame, text="Schwarz", variable=self.pawn_color, value="black", font=("Arial", 16), bg="black", fg="gray", indicatoron=True).pack(anchor="w", padx=20, pady=5)
       ...
       # Separate frame for the Quit Button
        ...
        for row in range(8):
            for col in range(8):
                ...
                # Annotate board with letters and numbers
                if row == 7:  # Bottom row for columns
                    square.create_text(self.square_size - 10, self.square_size - 10, text=chr(97 + col), font=("Arial", 12), fill="black")
                if col == 0:  # Left column for rows
                    square.create_text(10, 10, text=str(8 - row), font=("Arial", 12), fill="black")
    ...
    def display_move_in_output(self, move):
        # Convertir le mouvement en notation UCI et modifier l'affichage
        uci_move = move.uci()
        piece_symbol = self.pieces.get(self.board.piece_at(move.from_square).symbol(), '')
        display_move = f"{piece_symbol}{uci_move[2:]}"
        ...
        if self.board.is_castling(move):
            if move.to_square > move.from_square:
                display_move = "0-0"  # Short castle
            else:
                display_move = "0-0-0"  # Long castle
    
        # Ajouter le mouvement à la zone de texte
        self.output_text.configure(state='normal')  # Déverrouiller la zone de texte
        self.output_text.insert(tk.END, f"\n{display_move}")  # Ajouter le mouvement en notation modifiée
